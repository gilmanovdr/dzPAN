...
const double TU134_NOMINAL_THRUST = 2 * 117680.0; // Суммарная тяга, Н
const double TU134_CY0 = 0.2;               // Коэффициент подъемной силы при нулевом угле атаки
const double MAX_THRUST_PERCENT = 1;     // Реалистичный процент тяги
const double INITIAL_ALTITUDE = 300.0;      // Начальная высота, м
const double FINAL_ALTITUDE = 6000.0;       // Конечная высота, м
const double INITIAL_VELOCITY = 310.0 / 3.6; // Начальная скорость, м/с
const double FINAL_VELOCITY = 700.0 / 3.6;  // Конечная скорость, м/с
...
public:
    ...
    void plotTrajectory() const {
        ...
        fprintf(gp, "set multiplot layout 2,2 title 'Параметры полета ТУ-134'\n");
...
public:
    ...
    double calculate_podiem(double initial_alt, double final_alt, double velocity, Aircraft& ac){
        ...
        double sin_tetha = min((current_P-X) / (TU134_MASS* GRAVITY), MAX_CLIMB_ANGLE/DEG_TO_RAD);
